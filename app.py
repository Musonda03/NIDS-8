...
import threading
import packet_capture  # Make sure packet_capture is properly imported
...
#traffic_data = {
    ...
    #"time": [int(time.time()) - 60 * i for i in range(5)]
#}

#anomalies = [
    #{"timestamp": int(time.time()) - 240, "packet_count": 30},
    #{"timestamp": int(time.time()) - 180, "packet_count": 40}
#]
anomalies = []
traffic_data = {
    "time": [],
...
def generate_anomalous_traffic_route():
    packet_capture.generate_anomalous_traffic()
